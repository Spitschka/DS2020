...
class Absenz {
  ...
  public function getLeistungsnachweiseDuringAbsenzPeriod() {
      ...
      return PortalApi::getLeistungsnachweise($this->getSchueler()->getAsvID(), $this->getDateAsSQLDate(), $this->getEnddatumAsSQLDate());
  }
  ...
  public function isSchriftlichEntschuldigbar() {
    if(DB::getSettings()->getValue('absenzen-fristabgabe-schriftliche-entschuldigung') > 0) {
      ...
      while(true) {
        ...
        if(!DateFunctions::isSQLDateWeekEnd($entschuldigungDate) && !PortalApi::isFerien($entschuldigungDate)) $daysAdded++;
  ...
  public function setJetztGekommen() {
    $stunde = PortalApi::getAktuelleStunde();
  ...
  public function getTotalDays() {
    ...
    while(DateFunctions::isSQLDateAtOrBeforeAnother($currentDay, $ende)) {
      ...
      if($tag != 0 && $tag != 6 && !PortalApi::isFerien($currentDay)) {
        if($klasse->isAnwesend(DateFunctions::getMySQLDateFromNaturalDate($start))) {
          $anzahl++;
        }
      }
  ...
  /**
   * Berechnet die Tage, die wirklich krank waren. (Bei denen die Klasse nicht anwesend war.)
   * @return int Tage
   */
   ...
   * @return int[] stats
   */
  public function getDaysToStats($stats) {
    $start = $this->data['absenzDatum'];
    $ende = $this->data['absenzDatumEnde'];
  ...
  public function getBeurlaubungTage() {
    ...
    while(DateFunctions::isSQLDateAtOrBeforeAnother($currentDay, $ende)) {
      ...
      if($tag != 0 && $tag != 6 && !PortalApi::isFerien($currentDay) && $this->isBeurlaubung()) $anzahl++;
