...
class DateFunctions {
	...
	public static function addOneDayToMySqlDate($date) {
		$data = DB::getDB()->query_first("SELECT DATE_ADD('$date', INTERVAL 1 DAY) AS nextDay;");
		return $data['nextDay'];
	}
	
	public static function addDaysToMySqlDate($date, $days) {
		$data = DB::getDB()->query_first("SELECT DATE_ADD('$date', INTERVAL $days DAY) AS nextDay;");
		return $data['nextDay'];
	}
	
	public static function substractOneDayToMySqlDate($date) {
		$data = DB::getDB()->query_first("SELECT DATE_ADD('$date', INTERVAL -1 DAY) AS prevDay;");
		return $data['prevDay'];
	}
