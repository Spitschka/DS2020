...
class krankmeldung extends AbstractPage {
	...
	public function execute() {		
		...
		if($_GET['mode'] == "saveKrankmeldung") {
			...
			while(DateFunctions::getWeekDayFromNaturalDate($today) == 0 || DateFunctions::getWeekDayFromNaturalDate($today) == 6 || PortalApi::isFerien(DateFunctions::getMySQLDateFromNaturalDate($today)))
			...
			if(DateFunctions::isNaturalDate($_POST['krankAb']) && DateFunctions::isNaturalDate($_POST['krankBis'])) {
				...
				$mvs = array("m","v","s");
				if(!in_array($_POST['krankmeldungDurch'], $mvs) && !DB::getSession()->isPupil()) {
					new errorPage("Falsche Angabe bei Mutter / Vater / Sonstiger!");
				}
				
				if(DB::getSession()->isPupil()) {
					if(DB::getSession()->getPupilObject()->getAlter() >= 18) {
						$_POST['krankmeldungDurch'] = 'scehuelerue18';
					}
					else {
						$_POST['krankmeldungDurch'] = 'scehueleru18';
					}
				}
				...
				if(DB::getSettings()->getBoolean("krankmeldung-hinweis-lnw") && stundenplandata::getCurrentStundenplan() != null) {
					...
					$allGrades = stundenplandata::getCurrentStundenplan()->getAllMyPossibleGrades($kind->getKlasse());
					$lnws = Leistungsnachweis::getByClass($allGrades, DateFunctions::getMySQLDateFromNaturalDate($_POST['krankAb']), DateFunctions::getMySQLDateFromNaturalDate($_POST['krankBis']));
	...
	private function showNewKrankmeldungForm($errorMessage="") {
		...
		if(!$skipNextDay) {
			...
			while(DateFunctions::getWeekDayFromNaturalDate($nextDay) == 0 || DateFunctions::getWeekDayFromNaturalDate($nextDay) == 6 || PortalApi::isFerien(DateFunctions::getMySQLDateFromNaturalDate($nextDay)) != null) {
				$nextDay = DateFunctions::getNaturalDateFromMySQLDate(DateFunctions::addOneDayToMySqlDate(DateFunctions::getMySQLDateFromNaturalDate($nextDay)));
			}
